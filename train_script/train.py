...
def train():
    ...
    dataset = load_dataset(args.datafile_paths[0], split="train")
    len_dst = len(dataset)
    dataset = dataset.select(range((len_dst // 10) *3))
    ...
    decay_parameters = get_parameter_names(state.model, [nn.LayerNorm])
    decay_parameters = [name for name in decay_parameters if "bias" not in name]
    optimizer_grouped_parameters = [
        {
            "params": [p for n, p in state.model.named_parameters() if n in decay_parameters],
            "weight_decay": training_args.weight_decay,
        },
        {
            "params": [p for n, p in state.model.named_parameters() if n not in decay_parameters],
            "weight_decay": 0.0,
        },
    ]
